...
void CommandController(enum SPECIAL_CHARACTER input_parse, char c, char buffer[], int * counter){
	...
	if(input_parse == BACK_SPACE){
		...
		uart_send(c);
		if(*counter == MAX_BUFFER_LEN) InputBufferOverflowMessage(buffer);
		else{
			buffer[(*counter)] = '\0';
...
emum SPECIAL_CHARACTER Parse(char){
	if(!(c < 128 && c >= 0)) return UNKNOWN;
	
	if(c == BACK_SPACE) return BACK_SPACE;
	else if(c == LINE_FEED || c == CARRIAGE_RETURN) return NEW_LINE;
	else return REGULAR_INPUT;
